...
// Garbage Collection Job Types

// GCJob represents a garbage collection job configuration
type GCJob struct {
	ID       string `json:"id"`
	Store    string `json:"store"`
	Schedule string `json:"schedule"`
...
func (c *Client) ListGCJobs(ctx context.Context) ([]GCJob, error) {
	resp, err := c.api.Get(ctx, "/config/garbage-collection")
	if err != nil {
		return nil, fmt.Errorf("failed to list GC jobs: %w", err)
	}

	var jobs []GCJob
	if err := json.Unmarshal(resp.Data, &jobs); err != nil {
		return nil, fmt.Errorf("failed to unmarshal GC jobs: %w", err)
	}

	return jobs, nil
}

// GetGCJob gets a specific GC job by ID
func (c *Client) GetGCJob(ctx context.Context, id string) (*GCJob, error) {
	path := fmt.Sprintf("/config/garbage-collection/%s", url.PathEscape(id))
	resp, err := c.api.Get(ctx, path)
	if err != nil {
		return nil, fmt.Errorf("failed to get GC job %s: %w", id, err)
	}

	var job GCJob
	if err := json.Unmarshal(resp.Data, &job); err != nil {
		return nil, fmt.Errorf("failed to unmarshal GC job %s: %w", id, err)
	}

	return &job, nil
}

// CreateGCJob creates a new GC job
func (c *Client) CreateGCJob(ctx context.Context, job *GCJob) error {
	if job.ID == "" {
		return fmt.Errorf("job ID is required")
	}
	if job.Store == "" {
	...
	if err != nil {
		return fmt.Errorf("failed to create GC job %s: %w", job.ID, err)
	}

	return nil
}

// UpdateGCJob updates an existing GC job
func (c *Client) UpdateGCJob(ctx context.Context, id string, job *GCJob) error {
	if id == "" {
		return fmt.Errorf("job ID is required")
	}
	...
	_, err := c.api.Put(ctx, path, body)
	if err != nil {
		return fmt.Errorf("failed to update GC job %s: %w", id, err)
	}

	return nil
}

// DeleteGCJob deletes a GC job
func (c *Client) DeleteGCJob(ctx context.Context, id string) error {
	if id == "" {
		return fmt.Errorf("job ID is required")
	}
	...
	_, err := c.api.Delete(ctx, path)
	if err != nil {
		return fmt.Errorf("failed to delete GC job %s: %w", id, err)
	}
